...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item" aria-current="page">{{ __('My Document') }}</li>
@endsection
    ...
    <div class="row">
        <div class="col-12">
            <div class="card table-card">
                <div class="card-header">
                    <div class="row align-items-center g-2 flex-wrap">
                        <div class="col-12 col-md">
                            <h5>{{ __('My Document') }}</h5>
                        </div>
                        <div class="col-12 col-md-auto">
                            {{ Form::open(['method' => 'get', 'route' => 'document.my-document', 'id' => 'document-my-table', 'class' => 'attendance-date']) }}
                            <div class="d-flex flex-wrap gap-2 align-items-end">

                                <div class="d-flex flex-column" style="width: 200px">
                                    <b>{{ Form::label('category', __('Category'), ['class' => 'form-label']) }}</b>
                                    {{ Form::select('category', $category, null, ['class' => 'form-control select2']) }}
                                </div>

                                <div class="d-flex flex-column" style="width: 200px">
                                    <b>{{ Form::label('stage', __('Stage'), ['class' => 'form-label']) }}</b>
                                    {{ Form::select('stage', $stages, null, ['class' => 'form-control select2']) }}
                                </div>
                ...
                <div class="card-body pt-0" id="document-my-report">
...
@push('script-page')
    <script>
        ...
        $(document).ready(function() {
            $('#document-my-table').on('submit', function(e) {
                e.preventDefault();

                $.ajax({
                    url: '{{ route('document.my-document') }}',
                    method: 'GET',
                    data: $(this).serialize(),
                    headers: {
                    ...
                    success: function(response) {
                        $('#document-my-report').html(response.html);
                        datatable();
                    },
                    error: function(xhr) {
            ...
            $('#document-my-table').trigger('submit');
        });
    </script>
@endpush
